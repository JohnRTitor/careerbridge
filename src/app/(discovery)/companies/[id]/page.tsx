...
import { getCompanyCached } from "@/features/companies/api/server-cached";
...
async function CompanyDetailsContent({
  ...
  try {
    const [companyResult, jobsResult] = await Promise.all([
      getCompanyCached({ companyId: id }),
      searchJobsCached({ companyId: id, limit: 10, page: 1, status: "open" })
    ]);
  ...
  return (
    <>
      <div className="flex flex-col md:flex-row gap-8 items-start md:items-center justify-between mt-6">
    ...
    <div className="flex flex-col min-h-screen bg-background pb-16">
      <div className="bg-background border-b border-border pt-8 pb-12">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <Suspense fallback={<div className="py-12 text-center text-muted-foreground">Loading company details...</div>}>
